#!/usr/bin/env bash
# Sweep #3c: corrected learning rate AND full module coverage.
#
...
#
# Sweep #3b was killed at epoch 5 because a second defect turned up while it ran. Each adapter is 400
# tokens over 50 modules; --max-tokens 128 kept the PREFIX, so 16 modules were kept and the same 34
# were dropped from every adapter. The reader had never seen two thirds of any adapter. Token
# selection is now a round-robin across modules, so the same 128-token budget covers all 50 and a
# smaller budget costs directions per module rather than whole modules.
#
...
( while true; do
    gcloud storage rsync -r results/sweep3c/ gs://ditloracle-corpus/reader/sweep3c/ >/dev/null 2>&1
    sleep 300
  done ) & SYNC=$!
...
run () { local gpu=$1; local name=$2; shift 2
  CUDA_VISIBLE_DEVICES=$gpu nohup python -u scripts/train_reader.py \
    ...
    --out "results/sweep3c/${name}.json" "$@" > "results/sweep3c/${name}.log" 2>&1 &
  echo "  gpu$gpu -> $name"; }
...
gcloud storage rsync -r results/sweep3c/ gs://ditloracle-corpus/reader/sweep3c/ 2>&1 | tail -1
echo "=== sweep3c complete ==="
