...
\title{DiT-LoRAcle: describing image diffusion adapters in natural language}
...
tokens. Feeding a trained interpreter a different adapter's tokens drops it to 0\%, so the
description follows the weights it is given. Accuracy rises with the training budget and has not
...
A LoRA adapter \citep{hu2022lora} is a small set of weight matrices that changes what a generative
model produces. A public hub holds more of them than anyone can run, and running one is the only
routine way to find out what it does. Reading the weights instead is cheap, and at that scale the
difference decides whether screening every uploaded adapter is possible at all.

Existing weight-space readers answer with a label. \citet{africa2026csam} detect harmful adapters
from the top-left singular direction with logistic regression. \citet{puertolas2026backdoors} detect
backdoors from spectral statistics of the update. \citet{han2026w2t} tokenise adapters after
canonicalising them. Each returns a label from a set fixed before the adapter was seen, so an adapter
implementing something outside that set is reported as the nearest label inside it. That is the wrong
shape for screening a hub, where the interesting adapter is the one nobody anticipated.
...
trains that language model to answer questions about the adapter. Two properties of the setting shape
the design. The adapter modifies a diffusion transformer \citep{peebles2023dit} while the interpreter
is a language model, so an adapter direction is not a vector in the interpreter's activation space
and cannot simply be handed over. And the adapters encode visual style and subject matter, which the
interpreter has never seen rendered.

We take the injection-and-question-answering shape from LoRAcle \citep{selder2026loracle}, which
reads text-model adapters with a language model of the same architecture. Section~\ref{sec:method}
states what we kept and what the change of modality forced us to replace.

\paragraph{Contributions.}
\begin{enumerate}\itemsep2pt
\item DiT-LoRAcle, a meta-model that describes image diffusion transformer adapters in natural
language with an interpreter of a different architecture and width from the model being read,
reaching 55.2\% on unseen adapters against 0\% for a matched control (Sections~\ref{sec:method}
and~\ref{sec:results}).
...
result, derived from four of our own failed runs (Section~\ref{sec:protocol}).
\end{enumerate}
...
these read a running model's internal state. DiT-LoRAcle reads a static artefact instead, the
...
places those tokens in the interpreter's residual stream, and a supervision scheme. Only the
interpreter is trained.
...
Reading an adapter with a same-architecture model allows each direction to be mapped through the base
model's own write-back matrix so it lands in the reader's residual stream. We built the analogue for
klein and found it does nothing: every klein module already carries one side at residual width, with
input-side modules such as \texttt{img\_mlp.0} ($[18432, 3072]$) carrying it on the input and
output-side modules such as \texttt{img\_mlp.2} ($[3072, 9216]$) on the output. Selecting that side
by dimension leaves the map with nothing to multiply, and the measured ratio $\lVert Wd \rVert /
\lVert d \rVert$ is exactly 1. Selecting it by module name instead, which is how our implementation
began, silently discarded 42.1\% of modules, because klein's names match none of the patterns used
for other diffusion backbones.
...
At the interpreter's scale the ordering inverts. Fitting one classifier per feature on the
interpreter's own split (625 adapters, 98 held out, chance 0.0083):

\begin{center}
\begin{tabular}{lccc}
\toprule
Feature & Held-out & $\times$ chance & Top-5 \\
\midrule
Subspace projectors & 0.031 & 3.7 & 0.031 \\
Top singular direction + logistic regression & 0.061 & 7.3 & 0.163 \\
\textbf{Bilinear sketch of $\Delta W$} & \textbf{0.092} & \textbf{11.0} & \textbf{0.194} \\
\bottomrule
\end{tabular}
\end{center}

Subspace projectors win the clamped test outright and come last here. This is why
...
\begin{figure}[t]
\centering
\includegraphics[width=0.74\linewidth]{figures/fig1_epoch_threshold.pdf}
...
Held-out adapters are unseen, one per concept, $n=105$.}
\label{fig:epochs}
\end{figure}
...
adapter's tokens. Retrieval rank is the normalised rank of the true concept, where 0.5 is chance and
lower is better. Chance accuracy is 0.0065.}
\label{tab:main}
...
unsupervised. Accuracy had not saturated at the largest budget we ran, so we do not know where the
...
and name what it does, at 55.2\% on unseen adapters against 0\% for a matched control, without ever
